let service = (app, ctx) => {
    async function searchByPaging(criteria) {
        ...
        return {
            results: {
                data: data,
                head: ["Entry ID", "Entry Time", "Customer", "Equipment Type", "Equipment#", "Load Status", "Load Status Changed", "Current Location", "Duration"],
                shortHead: ["Entry ID", "Entry Time", "Equipment#", "Load Status", "Load Status Changed", "Duration"]
            },
    ...
    async function _buildEquipmentReportData(equipments) {
        ...
        _.each(equipments, e => {
            let equipment = {
                ...
                "Load Status Changed": e.updatedWhen ? commonService(ctx).ensureNotNull(momentZone(e.updatedWhen).format('YYYY-MM-DD HH:mm:ss')) : "",
                "Current Location": commonService(ctx).ensureNotNull(currentLocationMap[e.currentLocationId]),
                "Duration": commonService(ctx).calculateDuration(e.updatedWhen, momentZone())
            };
            data.push(equipment);
        });
        return data;
